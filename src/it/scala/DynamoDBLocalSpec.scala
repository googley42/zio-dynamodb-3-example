...
abstract class DynamoDBLocalSpec extends ZIOSpec[DynamoDBExecutor] {
  ...
  private val dynamoDbLayer: ZLayer[Any, Throwable, DynamoDb] =
    (netty.NettyHttpClient.default ++ awsConfig) >>> config.AwsConfig.default >>> dynamodb.DynamoDb
      .customized { builder =>
        builder
  ...
  override def bootstrap: ZLayer[Any, Nothing, DynamoDBExecutor] =
    dynamoDBExecutorLayer.orDie
  ...
  final def managedTable(
    ...
    ZIO
      ...
      )(tName => DynamoDBQuery.deleteTable(tName).execute.orDie)
  ...
  def withSingleIdKeyTable(
    ...
    ZIO.scoped {
      managedTable(singleIdKeyTable).flatMap(t => f(t))
    }
  ...
  def withIdAndYearKeyTable(
    ...
    ZIO.scoped {
      managedTable(idAndYearKeyTable).flatMap(t => f(t))
    }
  ...
  def withIdAndAccountIdGsiTable(
    ...
    ZIO.scoped {
      managedTable(idAndAccountIdGsiTable).flatMap(t => f(t))
    }
